...
ooperation_reward_record operation_reward_record::lookup( const chain_interface& db, const operation_id_type id )
{ try {
    return db.operation_reward_lookup_by_id( id );
} FC_CAPTURE_AND_RETHROW( (id) ) }

void operation_reward_record::store( chain_interface& db, const operation_id_type id, const operation_reward_record& record )
{ try {
    db.operation_reward_insert_into_id_map( id, record );
} FC_CAPTURE_AND_RETHROW( (id)(record) ) }

void operation_reward_record::remove( chain_interface& db, const operation_id_type id )
{ try {
