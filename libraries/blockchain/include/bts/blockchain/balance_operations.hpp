...
struct note_operation
{
    ...
    enum packet_condition_type
    {
        packet_rp_type          = 0,
        packet_group_type       = 1
    };
    
    struct packet_rp_condition
    {
        static const packet_condition_type type;
        
        packet_rp_condition(){}
        
        packet_rp_condition(share_type r): rp(r)
        {}
    ...
    // TODO: group_rp
    // struct packet_group_condition
    
    struct packet_claim_condition
    {
        fc::enum_type<fc::unsigned_int,packet_condition_type> type = packet_rp_type;
        vector<char>                                 data;
        
        packet_claim_condition( packet_condition_type atype = packet_rp_type )
        :type( atype ){}
        
        template<typename PacketConditionType>
        packet_claim_condition(  const PacketConditionType& t )
        :type( PacketConditionType::type )
        {
            data = fc::raw::pack( t );
        }
        
        template<typename PacketConditionType>
        PacketConditionType as()const
        {
            FC_ASSERT( type == PacketConditionType::type, "", ("PacketConditionType",PacketConditionType::type) );
            return fc::raw::unpack<PacketConditionType>(data);
        }
    };
    
    struct red_packet_operation
    {
        static const operation_type_enum type;
        ...
        string                              message;
        // claim_public_key is one time key generated by random_id+password
...
FC_REFLECT_ENUM( bts::blockchain::packet_condition_type,
                (packet_rp_type)
                (packet_group_type)
                )
FC_REFLECT( bts::blockchain::packet_rp_condition,
           (rp)
           )
FC_REFLECT( bts::blockchain::packet_claim_condition,
           (type)
           (data)
           )
...
FC_REFLECT( bts::blockchain::red_packet_operation, (random_id)(amount)(from_account_id)(message)(claim_public_key)(count)(claim_condition) )
FC_REFLECT( bts::blockchain::claim_packet_operation, (random_id)(to_account_id) )
