...
namespace bts { namespace game {
   ...
   public:
      ...
      public:
         ...
         virtual void execute( chain_database_ptr blockchain,
                                     uint32_t block_num,
                                     const pending_chain_state_ptr& pending_state ) = 0;
      };
      ...
      public:
         rule_converter()
         {
            _engine = std::make_shared< v8_game_engine > (RuleType::type);
         }
         ...
         { try {
            return _engine->scan_result(rtrx, block_num, block_time, trx_index, w);
         } FC_CAPTURE_AND_RETHROW( ) }
              
         virtual void execute( chain_database_ptr blockchain, uint32_t block_num, const pending_chain_state_ptr& pending_state )
         { try {
            return _engine->execute(blockchain, block_num, pending_state);
         } FC_CAPTURE_AND_RETHROW( ) }
      private:
         std::shared_ptr<v8_game_engine> _engine;
      };
